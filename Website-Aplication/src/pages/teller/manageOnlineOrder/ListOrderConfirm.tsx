...
const ListOnlineConfirm = () => {
  const dispatch = useDispatch();
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const onlineOrderInfo: listOnlineOrder<string, number> = useSelector((state: selectorOnlineOrder<string, number>) => state.onlineOrderReducer.onlineOrderConfirm);
    ...
    if (addSearch !== "") {
      if (chooseSelect === "name") {
        const data = {
          ...
          orderStatus: statusOrder.Confirm,
        }
...
export default ListOnlineConfirm
