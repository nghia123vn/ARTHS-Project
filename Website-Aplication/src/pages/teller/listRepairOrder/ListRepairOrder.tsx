...
const ListRepairOrder = () => {
        ...
        if (addSearch !=="") {
            if (chooseSelect === "name") {
                const data = {
                    ...
                    orderStatus: statusOrder.Repairing,
                }
            ...
            dispatch(getOrderPaid(paginationNumber, statusOrder.Repairing));
            setIsLoading(true);
        }
    ...
    useEffect(() => {
        const isAnyPendingOrder = orderPaidInfor?.data?.every((item) => item.status === statusOrder.Repairing);
