...
const ListStaff = () => {
    ...
    const staffInfor: itemAccount[] = useSelector((state: selectorListEmployee) => state.listEmployeeReducer.employeeStaffInfor);
    ...
    useEffect(() => {
        if (staffInfor.length && addSearch) {
            setCurrentPage(0);
            navigate('?page=1');
            setIsLoading(true)
        }
    ...
    }, [addSearch, navigate, staffInfor.length]);
    useEffect(() => {
        const data = {
            fullName:addSearch,
            status: typeAccount.InActive,
        }
        dispatch(getEmployee(roleAccount.Staff,data));
        setIsLoading(true);
