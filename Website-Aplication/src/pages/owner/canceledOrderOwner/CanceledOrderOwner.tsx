...
import TableOrderOnlineOrder from "@/components/owner/TableOrderOnlineOwner";
...
const CanceledOrderOwner = () => {
    const dispatch = useDispatch();
    const [isLoading, setIsLoading] = useState<boolean>(false);
    const onlineOrderInfo: listOnlineOrder<string, number> = useSelector((state: selectorOnlineOrder<string, number>) => state.onlineOrderReducer.onlineOrderCanceled);
                ...
                <div>
                    {onlineOrderData?.length > 0
                        ? (<div className={`${onlineOrderData?.length < 12 ? "h-[65vh]" : ""}`}>
                            <TableOrderOnlineOrder data={onlineOrderData} />
                        </div>)
                        : (
...
export default CanceledOrderOwner
