...
import { navigateToCalendarScreen, navigateToDetailOrderScreen } from "@/utils/navigation";
...
import { requestLoadMoreProducts } from "@/store/product/function";
    ...
    <Swipeable renderRightActions={renderRightActions}>
      <SContentBox onPress={async () => {
        ...
        if (data?.data.link) {
          ...
          if (data?.data.type === "MaintanenceSchedule") {
  ...
  return (
    <FullScreenWrapper>
      ...
      <SContainer>
        ...
        <FlatList showsVerticalScrollIndicator={false} data={listNotifications} renderItem={renderItem}
                  refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}
                  onEndReached={handleLoadMore}
                  onEndReachedThreshold={0.5}
                  ListHeaderComponent={listNotifications.length > 0 ? <TouchableOpacity onPress={onMarkAll}>
                    <STextBold style={{
                      color: "#0d45d2",
                      textAlign: "right",
                      fontSize: 14
